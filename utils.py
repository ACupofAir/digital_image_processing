...
def open_image(name):
    try:
        ...
        print("---------------the file is not in fold images--------------------")
        img = Image.open(name)
    return img


def get_image(channel="L", imgname="../images/sample.jpg"):
    """
    @paramete 
    @param channel : "L" or "RGB"
    @param imgname : path to the image
    @return np.float32 array
    """
    img = open_image(imgname)
    if channel == "L":
        img = img.convert("L")
    elif channel == "RGB":
        img = img.convert("RGB")
    else:
        print("---------------unaivialable paramete----------------------------")
        img = img
...
class visualization:
    """
    一个视觉对象，可以往里面加图片，最后半智能的plot出来
    @paramete 
    @param img : np.float32 array
    """
...
def test_util():
    print("---------------------utils has been imported successly---------------")
